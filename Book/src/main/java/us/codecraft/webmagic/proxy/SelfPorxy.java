 ...
 * @since 0.5.1
 */
public class SelfPorxy implements ProxyPool {
    ...
    public SelfPorxy(List<String[]> httpProxyList) {
        this(httpProxyList, true);
    }

    public SelfPorxy(List<String[]> httpProxyList, boolean isUseLastProxy) {
    ...
    public void returnProxy(HttpHost host, int statusCode) {
        ...
        if (p.getFailedNum() > 20) {
            ...
            deleteProxy(host.getAddress().getHostAddress(),host.getPort());
    ...
    public void deleteProxy(String ip, int port){
    	
	
		SqlDao dao =new SqlDao(conn);
    	dao.deleteProxyIP(ip, String.valueOf(port));
